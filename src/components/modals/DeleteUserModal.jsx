...
import { useDeleteUser } from '../../hooks/useDashboardQueries';
...
export default function DeleteUserModal() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(`تم حذف حساب المستخدم "${modalData?.name}" نهائياً بنجاح`);
      closeModal();
    } catch (err) {
      ...
      <div className="mono-card w-full max-w-md p-6 space-y-6 shadow-2xl relative text-right dir-rtl">
        ...
        <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-white">تأكيد حذف حساب المستخدم</h2>
          </div>
          <button 
            onClick={closeModal}
        ...
        <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl flex items-start gap-3 text-rose-400 text-xs leading-relaxed">
          ...
          <div>
            ...
            هل أنت متأكد من حذف حساب المستخدم <strong className="text-white">"{modalData?.name}"</strong> ({modalData?.email}) نهائياً من المنظومة؟
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-zinc-800">
          ...
          >
            {deleteUserMutation.isPending ? 'جاري الحذف...' : 'تأكيد الحذف النهائي'}
          </button>
        </div>
